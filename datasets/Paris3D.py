...
# ----------------------------------------------------------------------------------------------------------------------
#
#      Class handling Pairs3D dataset.
...
class Paris3DDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        ...
        """
        PointCloudDataset.__init__(self, 'Paris3D')
        ...
        if self.set == 'training':
            ...
            raise ValueError('Unknown set for Paris3D data: ', self.set)
        ...
        for i, f in enumerate(self.cloud_names):
            if self.set == 'training':
                ...
                raise ValueError('Unknown set for Paris3D (with features) data: ', self.set)
...
class Paris3DSampler(Sampler):
    """Sampler for Paris3D"""

    def __init__(self, dataset: Paris3DDataset):
...
class Paris3DCustomBatch:
    """Custom batch definition with memory pinning for Paris3D"""
...
def Paris3DCollate(batch_data):
    return Paris3DCustomBatch(batch_data)
