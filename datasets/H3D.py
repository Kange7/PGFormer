# ----------------------------------------------------------------------------------------------------------------------
#
#      Class handling Hessigheim 3D  dataset.
...
class H3DDataset(PointCloudDataset):
    ...
    def __init__(self, config, set="training", use_potentials=True, load_data=True, path = "../Data/Hessigheim_Benchmark/Epoch_March18"):
        ...
        for i, f in enumerate(self.cloud_names):
            if self.set == "training":
                if self.all_splits[i] == self.train_splits:
                    self.files += [join(ply_path, f + ".ply")]
            elif self.set in ["validation", "ERF"]:
                if self.all_splits[i] == self.validation_split:
                    self.files += [join(ply_path, f + ".ply")]
            elif self.set == 'test':
                if self.all_splits[i] == self.test_splits:
                    self.files += [join(ply_path, f + '.ply')]
            else:
                raise ValueError("Unknown set for H3D data: ", self.set)
    ...
    def potential_item(self, batch_i, debug_workers=False):
        ...
        while True:
            ...
            # input_reflect = self.input_reflect[cloud_ind][input_inds]
...
def H3DCollate(batch_data):
    return H3DCustomBatch(batch_data)
