...
# ----------------------------------------------------------------------------------------------------------------------
#
#      Based on the work of Hugues THOMAS
#
...
def accuracy_from_confusions(confusions):
    """
    Computes accuracy from confusion matrices.
    :param confusions: ([..., n_c, n_c] np.int32). Can be any dimension, the confusion matrices should be described by
    the last axes. n_c = number of classes
    :return: ([..., n_c] np.float32) accuracy score for each class and overall accuracy
    """

    # Compute TP, FP, TN, FN
    TP = np.diagonal(confusions, axis1=-2, axis2=-1)
    FP = np.sum(confusions, axis=-2) - TP  # False positives
    FN = np.sum(confusions, axis=-1) - TP  # False negatives
    TN = np.sum(confusions) - (TP + FP + FN)  # True negatives
    TP_plus_FN = np.sum(confusions, axis=-1)

    # Calculate accuracy for each class
    accuracy_per_class = (TP + TN) / (TP + TN + FP + FN + 1e-6)  

    # Overall accuracy
    # counts = np.sum(TP + TN + FP + FN, axis=-1, keepdims=True)
    # overall_accuracy = (TP + TN).sum(axis=-1) / (counts + 1e-6)
    overall_accuracy = (TP).sum(axis=-1) / (TP + TN + FP + FN + 1e-6)

    # Calculate mAcc (mean accuracy across classes)
    # mAcc = accuracy_per_class.mean(axis=-1)  # Average accuracy across all classes
    # mask = TP_plus_FN < 1e-3
    # counts = np.sum(1 - mask, axis=-1, keepdims=True)
    # mAcc = np.sum(accuracy_per_class, axis=-1, keepdims=True) / 13
...
def F1_from_confusions(confusions):
    """
    Computes F1 score from confusion matrices.
    :param confusions: ([..., n_c, n_c] np.int32). Can be any dimension, the confusion matrices should be described by
    the last axes. n_c = number of classes
    :return: ([..., n_c] np.float32) F1 score for each class
    """
    # Compute TP, FP, FN
    TP = np.diagonal(confusions, axis1=-2, axis2=-1)
    FP = np.sum(confusions, axis=-2) - TP
