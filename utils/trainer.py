...
# Metrics
from utils.metrics import IoU_from_confusions, fast_confusion, accuracy_from_confusions, F1_from_confusions
...
class ModelTrainer:
    ...
    def cloud_segmentation_validation(self, net, val_loader, config, debug=False):
        ...
        F1s = F1_from_confusions(C)

        t5 = time.time()

        #  ACC
        accuracy_per_class, overall_accuracy = accuracy_from_confusions(C)
        ...
        if config.saving:
            ...
            # acc_list = [overall_accuracy]  
            # acc_file = 'overall_accuracy.txt'  

            test_file2 = join(config.saving_path, 'val_F1s.txt')
            ...
            # Write in file
            if exists(test_file2):
                with open(test_file2, "a") as text_file2:
                    text_file2.write(line2)
            else:
                with open(test_file2, "w") as text_file2:
                    text_file2.write(line2)
            ...
            # print(type(mAcc))
            acc_line = '{:.3f}\t{:.3f}\n'.format(overall_accuracy[0])  
            acc_test_file = join(config.saving_path, 'accuracy.txt')

            # Write accuracy to file
            if exists(acc_test_file):
                with open(acc_test_file, "a") as text_file:
                    text_file.write(acc_line)
            else:
                with open(acc_test_file, "w") as text_file:
                    text_file.write(acc_line)
