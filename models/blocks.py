...
from GRTmodel.Mym_seg import TransformerBlock
...
class ResnetBottleneckBlock(nn.Module):
    ...
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, config):
        ...
        # Other operations
        self.leaky_relu = nn.LeakyReLU(0.1)
        # self.transformer1 = TransformerBlock(out_dim // 4, 128, 4)
        self.transformer1 = TransformerBlock(out_dim // 4, 128)
    ...
    def forward(self, features, batch):
        ...
        x = self.KPConv(q_pts, s_pts, neighb_inds, x1)  
        # x = self.KPConv(q_pts, s_pts, neighb_inds, x)

        if 'strided' in self.block_name:
            x2 = max_pool(x1, neighb_inds)  #  features -> X1
        else:
            x2 = x1
            
        x = self.leaky_relu(self.batch_norm_conv(x + 0.25*x2))  
        ...
        # Shortcut  
        if 'strided' in self.block_name:
            shortcut = max_pool(features, neighb_inds)  #  features -> X1
        else:
            shortcut = features
